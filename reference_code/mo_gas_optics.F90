...
!   Get internal source functions at layers and levels, which depend on mapping from spectral space that creates k-distribution.
  ...
  function compute_gas_taus(this,                       &
    ...
    integer, dimension(2, this%get_nflav())          :: flavor          ! indices of two major absorbing gases per band,
                                                                        ! referring to gas-which-are-present
    ...
    do igas = 1, ngas
      ...
      ! gases are skipped in calculation
         vmr(:,:,igas) = 0._wp
      end if
    end do
    ...
    if(.false.) then
      ...
      !
      do igas = 1, ngas
        error_msg = gas_desc%get_vmr(terse_gas_names(igas), vmr(:,:,igas))
        if (error_msg /= '') return
      end do
      !
      ...
      allocate(terse_gas_names(size(this%gas_names)))
      terse_gas_names(:) = this%gas_names(:)
    end if
    !
    !
    !
